...
def run_os():
    os.system('ngspice -b interposer1_tr_new.sp -r interposer1_tr.raw')
...
def move(action, cap_idx):
    ...
    return action, cap_idx ,break_out
...
class Agent:
    ...
    def __init__(self, dcnum, action_space):
        ...
        self.memory = np.zeros([memorysize, (nstate + dcnum) * 2 + 10])      # reward = 5 , action = 5 ,so +10
    ...
    def initial_loc(self, dc_val):
        cap_idx = random.sample([a for a in range(nstate)], 5)
        cap_idx.sort()
        str = ''
    ...
    def step(self, a, cap_idx, dc_val):
        ...
        for i in range(len(a)):
            action[i], cap[i],br_out = move((a[i] % 5), cap_idx[i])
        ...
        #print(str_dc)
            ...
            run_os()
        state_ = readvdi('chiplet1_vdd_1_vdi.csv')
        self.bo.append(br_out_list)
        
        return state_, action, cap,br_out_list
    ...
    def update(self):
        ...
        batch_a = torch.LongTensor(batch_memory[:, nstate + 5:nstate + 10].astype(int))
        batch_r = torch.FloatTensor(batch_memory[:, nstate + 10:nstate + 15])
        batch_s_ = torch.FloatTensor(batch_memory[:, -nstate - self.dcnum:])

        # MSELoss
        np.savetxt('mem.txt', self.memory[:, nstate + 5:nstate + 10], fmt='%d')
        ...
        q_next_max = torch.zeros([batch_size, 5])
...
dnn = Agent(dcnum=5, action_space=50)
target_vdi = 1e-9  # 1.5 -> 1.2
reward = []
total_vdis = []
cap_list = []
cap_idx = np.zeros(5)
...
for eps in range(episodes):
    ...
    obs_, a, cap,break_out = dnn.step(a, cap_idx, 1e-9)
    a_change.append(a)
    a_array = np.array(a_change)  # modified action
    cap_list.append(cap)  # cap move data
    #r = (target_vdi - np.sum(obs_)) / target_vdi if break_out == 0 else (target_vdi - np.sum(obs_)) / target_vdi - break_out
    #r = (target_vdi - np.sum(obs_)) / target_vdi if break_out == 0 else -5

    update_r = np.zeros(5)
    for nr in range(len(update_r)):
        update_r[nr] = (target_vdi - np.sum(obs_)) / target_vdi if break_out[nr] == 0 else -5 
    s_ = norm(obs_, cap_idx)
    dnn.store_transition(s, a, update_r, s_)
    r = (target_vdi - np.sum(obs_)) / target_vdi
    reward.append(r)
    total_vdis.append(np.sum(obs_))
    obs = obs_
    cap_idx = cap
    if dnn.mem_cnt >= memorysize:
        if (eps + 1) % 50 ==0:
            #or_update = 0
...
np.savetxt('break_out.txt',dnn.bo,fmt='%d',delimiter='\t')
